...
Even React.cloneElement is not more preferable way of doing it (check
https://react.dev/reference/react/cloneElement#alternatives), I think it's the
only way to achieve compound components patters

From the alternative documentation component is receiving `render prop` which we
are using for rendering _same item components_ with additional props
